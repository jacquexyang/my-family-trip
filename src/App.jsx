  ...
  useEffect(() => {
     ...
     const unsubPacking = onSnapshot(query(packingRef, orderBy('createdAt')), (snapshot) => {
         if (!snapshot.empty) {
             ...
             // Init default packing list if empty logic could be here
             // Ensure this runs only once or check against a flag
             // For simplicity, we can rely on manual trigger or leave it empty for now.
             // If we want auto-init, we can do it here, but be careful of loops.
             // Let's do a simple check if we are in Seoul trip and list is empty
             if (tripId === 'seoul_2025' && packingList.length === 0 && !snapshot.metadata.hasPendingWrites) {
                // We can seed initial data here if needed, but useEffect runs on updates.
                // Better to seed when creating the trip or manually.
                // We'll leave it empty for now to avoid complexity or potential loops in this snippet.
             }
         }
     });
      ...
      <div className="relative w-full h-[40vh] md:h-[50vh]">
        ...
        <div className="absolute top-0 left-0 right-0 p-4 md:p-6 flex justify-between items-center z-10 max-w-7xl mx-auto w-full">
           ...
           <div className="relative">
             ...
             {isToolsOpen && (
               <div className="absolute top-12 right-0 w-48 bg-white rounded-2xl shadow-xl border border-stone-100 p-2 animate-in fade-in zoom-in duration-200 origin-top-right z-50">
